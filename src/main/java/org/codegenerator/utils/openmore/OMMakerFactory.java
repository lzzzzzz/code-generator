        ...
        try {
            ...
            if (null == base_package || "".equals(base_package)) {
                ...
                if (null == source_path || "".equals(source_path)) {
                    //自定义目录不存在则使用默认目录
                    out_file_dir = FreeMakerConfig.ROOT_PATH + File.separator + FreeMakerConfig.DEFAULT_SOURCES_DIRECTORY;
                } else {
                    out_file_dir = FreeMakerConfig.ROOT_PATH + File.separator + out_file_dir;
            ...
            //获取输出文件
            File out_file = new File(out_file_dir + File.separator + fileName);
